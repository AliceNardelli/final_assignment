...
# callbacks
#subscriber for getting actual position and oriantation of the robot
def clbk_odom(msg):
...
#subscriber for getting actual output of laser
#output are localized in the robot in 5 regions organized as at the bottom
#variable regions_ separes the output of lasers according how laser are organized in the robot 
def clbk_laser(msg):
    global regions_
    regions_ = {
...
def change_state(state):
    ...
    if state_ == 2:
        #stop behaviour, two services are set to false
        resp = srv_client_go_to_point_(False)
        resp = srv_client_wall_follower_(False)
...
def main():
    ...
    #subscriber of topic /scan for get output of laser
    sub_laser = rospy.Subscriber('/scan', LaserScan, clbk_laser)
    #subscriber of topic /odom for get actual position
    sub_odom = rospy.Subscriber('/odom', Odometry, clbk_odom)
    #publisher of topic /cmd_vel for stop the robot
    ...
    srv_client_wall_follower_ = rospy.ServiceProxy(
        '/wall_follower_switch_bug', SetBool)
		        ...
		        continue
				        ...
				        if elapsed_time>120 :
							...
							if state_ == 0:
							    ...
							    if(err_pos < 0.3):
								change_state(2)
                                                            #if there are no obstacles in front of the robot and orientation of robot is correct pass to go to point state
